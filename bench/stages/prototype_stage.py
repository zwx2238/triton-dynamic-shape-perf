...
from bench.bucket_tune.prototype_utils import (
...
def run_prototype(config: BenchmarkConfig, state: BenchmarkState) -> str:
    ...
    typical_shapes = pick_typical_shapes(config.tune_set, proto_count)
    proto_candidates, proto_rows = derive_candidate_pool_from_typical_shapes(
        typical_shapes,
        state.candidates,
    ...
    write_prototype_report_csv(config.prototype_report_csv, proto_rows)

    cfg_desc = ",".join(c.config_id for c in state.candidates)
